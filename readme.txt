...
위의 파일이 include되지 않을 경우 [프로젝트 속성 -> C/C++ -> 추가포함 디렉토리]에 프로젝트 파일 내의 lib/glm폴더를 포함시켜주셔야 정상작동합니다.
...
8. 날아오는 속도 변경(+/-)
	구현 방법 : 전체 도형의 속도값을 Speed(기본값 1.0f)로 저장하고 +/-입력시 Speed값을 변하게 하고 물체의 이동연산때 Speed값을 곱해주어 Speed값에 따라 물체의 이동속도가 달라지게 구현하였다.

A - 2. 보너스 요소
1. 여러번 슬라이스, 자유 슬라이스
	위의 4. 절취연산에서 선분과 도형의 교차점에 따라 왼쪽위 / 오른쪽아래로 도형이 분리되게 구현하여 슬라이스한 도형의 모양이 일정하지 않게 만들었고 슬라이스한 도형은 다시 onShape에 저장되어 슬라이스된 도형을 다시 슬라이스 할 수 있다.

2. 도형이 회전하며 날아오고 회전한 형태에서 슬라이스 가능
	도형의 회전같은 경우에는 TimerFunction(1)에서 기존에 onShape에 저장되어 있던 정점을 Rotation값에 따라 회전한 정점을 구하고 그 값을 다시 onShape에 저장하는 방식으로 하여 기존에 vertex.glsl에서 변환을 한 것과 다르게 저장되어 있는 정점의 좌표를 회전한 좌표로 바꾸는 것이기 때문에 슬라이스를 진행할 때 기존과 같은 방식으로 슬라이스를 진행해도 같은 결과를 얻을 수 있다.

3. 날아오는 경로의 다양화(랜덤한 경로)
	도형의 이동은 velocity를 location에 더하는 방식으로 진행되기 때문에 velocity의 값을 도형이 처음 생성되었을 때 랜덤한 값을 넣게 되면 모든 도형이 각기 다른 속도와 경로를 가지게 된다.

4. 바람 시스템 추가
	glm::vec3 Wind에 바람의 벡터를 저장하여 도형의 가속연산에 더해주는 방식으로 구현하였다. 추가로 화면의 왼쪽상단에 삼각형의 형태로 현재 바람의 방향과 크기를 표기해 주었는데 이는 atan2와 glm::length를 이용하여 바람의 각도와 크기를 구하여 roate와 scale에 사용해 표현하였다.

B. 구현하지 못한 내용
1. 도형이 '지나갈' 경로를 미리 출력하지는 못한다.
	도형의 속도값이 랜덤이기 때문에 도형이 자나갈 경로를 처음부터 미리 보여주지는 못한다.


C. 키보드/마우스 명령어
1. 마우스 클릭 - 드래그 - 클릭해제 : 클릭 해제시 선에 닿은 도형 슬라이스(최초 1개만)
2. p - 도형이 지나간 경로 출력, m - 도형의 모드 변경, +/- 도형의 속도 증가/감소, w - 바람 on/off, q - 프로그램 종료
